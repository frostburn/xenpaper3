...
Append `:output,input` to a connection target when a connection needs explicit
Web Audio channel ports. For example, `delay -> merger:0,1` connects output 0 of
`delay` to input 1 of `merger`; omitting the suffix continues to select the
default ports.
