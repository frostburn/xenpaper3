...
describe('directive runtime', () => {
  ...
  it('validates empty repeat boundaries once regardless of the repeat count', () => {
    const source = '@time(4/4) C |:@x50001 :|'
    const result = compile(source)
    const highlighted = result.diagnostics
      .filter(({ code }) => code === 'XP_BARLINE_OFF_CYCLE')
      .flatMap(({ locations }) => locations)
    ...
    expect(result.diagnostics).not.toContainEqual(
      expect.objectContaining({ code: 'XP_REPEAT_EXPANSION_LIMIT' }),
    )
    expect(highlighted).toEqual(['|:', ':|'])
  })
